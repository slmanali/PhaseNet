...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the real-valued PhaseNet model (refactored)."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Loss function weight parameter (v in Total_loss).",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def main():
    ...
    # Loss and optimizer
    criterion = Total_loss(v=args.loss_weight).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, triplets_batch in enumerate(pbar):
            try:
                # Stack batch: [B, 3, H, W] for each key
                ...
                # Process each color channel separately (as in original code)
                channel_losses = []
                ...
                for channel in range(3):
                    ...
                    pred_img_ch = pyr.reconstruct(
                        output_convert(pred_coeff),
                        pyr_type=pyr_type
                    )  # [B, H, W]
                    if pred_img_ch.dim() == 3:
                        pred_img_ch = pred_img_ch.unsqueeze(1)  # [B, 1, H, W]

                    truth_img_ch = inter_ch  # [B, 1, H, W]

                    # Compute loss for this channel
                    loss_ch = criterion(truth_coeff, pred_coeff, 
                                       truth_img_ch.squeeze(1), 
                                       pred_img_ch.squeeze(1))
                ...
                if total_step % args.log_interval == 0:
                    ...
                    print(log_msg)
                    print(f"  Model alpha: {model.state_dict()['alpha'].item():.4f}")
                ...
                if args.save_interval > 0 and total_step % args.save_interval == 0:
                    with torch.no_grad():
                        # Save first sample, first channel
                        vis_truth = inter_batch[0, 0, :, :].cpu().clamp(0, 1)
                        vis_pred = pred_img_ch[0, 0, :, :].cpu().clamp(0, 1)
                        
                        comparison = torch.stack([vis_truth, vis_pred], dim=0)
                        save_image(
                            comparison,
