...
Строки на сравнительном рисунке: эталон, ComplexPhaseNet, RIFE, FILM.
Входные кадры считываются для проверки происхождения, но не отображаются.
...
LABELS = (("gt", "Эталон"), ("complex", "ComplexPhaseNet"),
          ("rife", "RIFE"), ("film", "FILM"))
...
def main():
    ...
    p.add_argument("--line", type=float, nargs=4, metavar=("X0", "Y0", "X1", "Y1"),
                   help="Линия для отдельного графика профилей яркости; на сравнительном рисунке не отображается")
    p.add_argument("--show-line", action="store_true",
                   help="При необходимости также нарисовать линию на сравнительном рисунке (по умолчанию скрыта)")
    ...
    fig, axes = plt.subplots(len(LABELS), 3, figsize=(17, 13), constrained_layout=True)
    for row, (key, label) in enumerate(LABELS):
        ...
        if a.line is not None and a.show_line:
        ...
        if key == "gt":
            error.set_facecolor("white")
            error.text(.5, .5, "Эталон: ошибка = 0", ha="center", va="center",
                       transform=error.transAxes, fontsize=9)
        else:
    ...
    metadata = {"mode": a.mode, "sample_index": index, "official_list": str(list_file),
                ...
                "comparison_overlay_line": bool(a.line is not None and a.show_line),
